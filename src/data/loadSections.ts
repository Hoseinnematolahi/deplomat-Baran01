...
const isPlaceholder = (payload: SectionContent) =>
  payload?.title === 'Pending extraction' || payload?.text?.includes('Run npm run extract');
...
export async function loadSections(): Promise<SectionContent[]> {
  ...
  if (modules.every(isPlaceholder)) {
    console.warn(
      'PDF content is still placeholder. Ensure catalog/DB-General-Catalog-V4.8.pdf exists and run npm run extract before deployment.'
    );
  }
